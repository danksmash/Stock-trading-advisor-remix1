    ...
    <section id="sector-news-feed" className="bg-[#161B22] border border-gray-800 rounded p-2.5 flex flex-col">
      <div className="flex flex-wrap items-start justify-between gap-2 mb-2 pb-1.5 border-b border-gray-800">
        <div className="flex items-start gap-1.5 min-w-0">
          ...
          <div>
            <h2 className="text-[10px] font-bold text-gray-200 tracking-wide">半導体・NAND業界ニュース（日本語要約）</h2>
            <p className="text-[8px] text-gray-500 mt-0.5">見出しの内容を日本語で要約し、キオクシアへの関連を整理しています。</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-2">
        {news.length === 0 && (
          <div className="xl:col-span-3 rounded border border-gray-800 bg-[#0D1117] p-3 text-[10px] text-gray-400">
            最新ニュースを取得できませんでした。古い固定記事や推測記事は代替表示しません。
          </div>
        )}
        {news.slice(0, 6).map((item) => (
          <article key={item.id} className="bg-[#0D1117] p-2.5 rounded border border-gray-800/80 flex flex-col gap-1.5 min-w-0">
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              <span className="text-[9px] font-mono text-gray-400 font-semibold break-words">{item.source}</span>
              {item.importance === 'HIGH' && <span className="text-[8px] bg-red-950/60 text-red-400 border border-red-800/40 px-1 rounded font-bold">重要</span>}
              <span className="text-[8px] font-mono text-gray-500 ml-auto">{item.publishedAt}</span>
            </div>

            <h3 className="text-xs font-bold text-gray-100 leading-relaxed">{item.summary}</h3>
            <p className="text-[9px] text-gray-500 leading-relaxed"><span className="font-bold text-gray-400">原題：</span>{item.title}</p>

            <div className="bg-[#161B22] p-2 rounded border border-gray-800 text-[9px] leading-relaxed">
              <span className="text-blue-400 font-bold">キオクシアへの見方：</span>
              <span className="text-gray-300"> {item.kioxiaImpact}</span>
            </div>

            <div className="mt-auto flex flex-wrap items-center gap-1.5 pt-0.5">
              {item.tags.map((t) => <span key={t} className="text-[8px] font-mono bg-gray-900 text-gray-400 px-1.5 py-0.5 rounded border border-gray-800">#{t}</span>)}
              {item.url && <a href={item.url} target="_blank" rel="noreferrer" className="ml-auto inline-flex items-center gap-1 text-[8px] text-blue-400 hover:text-blue-300">原記事を開く <ExternalLink className="w-2.5 h-2.5" /></a>}
            </div>
          </article>
        ))}
      </div>
      <p className="mt-2 text-[8px] text-gray-500 leading-relaxed">※ 日本語要約と影響評価は速報見出しから自動生成した参考情報です。売買判断は価格・出来高・SOX・同業株など他の指標と合わせて行ってください。</p>
    </section>
  );
};
