    ...
    <section id="sector-news-feed" className="bg-[#161B22] border border-gray-800 rounded p-2.5 flex flex-col">
      <div className="flex flex-wrap items-start justify-between gap-2 mb-2 pb-1.5 border-b border-gray-800">
        <div className="flex items-start gap-1.5 min-w-0">
          ...
          <div>
            <h2 className="text-[10px] font-bold text-gray-200 tracking-wide">半導体・NAND業界ニュース（日本語）</h2>
            <p className="text-[8px] text-gray-500 mt-0.5">日本語ニュースフィードから直接取得しています。外部翻訳APIには依存しません。</p>
          </div>
        </div>
        <span className="text-[8px] font-mono text-emerald-400">日本語ソース</span>
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-3 gap-2">
        {news.length === 0 && (
          <div className="xl:col-span-3 rounded border border-gray-800 bg-[#0D1117] p-3 text-[10px] text-gray-400">
            現在、日本語の最新業界ニュースを取得できませんでした。英語記事を日本語訳済みとして代替表示することはしません。
          </div>
        )}
      ...
      <p className="mt-2 text-[8px] text-gray-500 leading-relaxed">※ 見出しは日本語ロケールのニュースフィードから取得したものです。センチメント分類とキオクシアへの影響評価は参考情報です。</p>
    </section>
  );
};
