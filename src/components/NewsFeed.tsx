...
export const NewsFeed: React.FC<NewsFeedProps> = ({ news }) => {
  ...
  React.useEffect(() => {
    ...
    if (!items.length) { setTranslations({}); return; }
    let cancelled = false;
    fetch('/api/translate-news', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({titles:items.map(i=>i.title)}) })
      ...
      .then(data => { if (cancelled || !Array.isArray(data?.translations)) return; const next:Record<string,string>={}; items.forEach((item,i)=>{ next[item.id]=String(data.translations[i] || item.title); }); setTranslations(next); })
  ...
  return <section id="sector-news-feed" className="bg-[#161B22] border border-gray-800 rounded p-2.5 flex flex-col">
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-2">
      ...
      {news.slice(0,6).map(item => { const jp=translations[item.id] || item.translatedTitle || item.title; return <article key={item.id} className="bg-[#0D1117] p-2.5 rounded border border-gray-800/80 flex flex-col gap-1.5 min-w-0">
        ...
        <h3 className="text-xs font-bold text-gray-100 leading-relaxed break-words">{jp}</h3>
        {jp!==item.title && <p className="text-[8px] text-gray-600 leading-snug">原題：{item.title}</p>}
    ...
    <p className="mt-2 text-[8px] text-gray-500 leading-relaxed">※ 日本語見出しは取得した原見出しの自動翻訳です。原題も併記し、別記事を同じ一般化見出しに置き換えない設計です。</p>
  </section>;
};
